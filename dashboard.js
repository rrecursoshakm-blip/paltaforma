...
const currentUser = _stored ? JSON.parse(_stored) : { name:'Admin AKM', email:'admin@akm.com', role:'Administrador', avatar:'A' };
...
document.getElementById('sidebarAvatar').childNodes[0].textContent = currentUser.avatar;
...
const defaultEmployees = [
  { id:1, name:'María García', cargo:'Gerente de RRHH', area:'Recursos Humanos', status:'active', date:'15/01/2020', salary:8500, color:'#6366f1', contractEnd:'2025-04-10', supervisor:null },
  { id:2, name:'Carlos López', cargo:'Analista de Nóminas', area:'Finanzas', status:'active', date:'03/06/2021', salary:5200, color:'#f97316', contractEnd:'2025-12-31', supervisor:1 },
  { id:3, name:'Ana Torres', cargo:'Coordinadora de Capacitación', area:'Desarrollo', status:'active', date:'12/03/2022', salary:6100, color:'#ec4899', contractEnd:'2025-08-15', supervisor:1 },
  { id:4, name:'Pedro Ramírez', cargo:'Jefe de TI', area:'Tecnología', status:'active', date:'08/09/2019', salary:9200, color:'#14b8a6', contractEnd:'2025-11-20', supervisor:null },
  { id:5, name:'Lucía Mendoza', cargo:'Asistenta Social', area:'Bienestar', status:'vacation', date:'22/11/2021', salary:4800, color:'#f59e0b', contractEnd:'2025-07-01', supervisor:1 },
  { id:6, name:'Roberto Díaz', cargo:'Contador Senior', area:'Finanzas', status:'active', date:'01/02/2018', salary:7600, color:'#8b5cf6', contractEnd:'2025-09-12', supervisor:2 },
  { id:7, name:'Carmen Rojas', cargo:'Reclutadora', area:'Recursos Humanos', status:'active', date:'15/07/2023', salary:4500, color:'#06b6d4', contractEnd:'2025-05-30', supervisor:1 },
  { id:8, name:'Diego Vargas', cargo:'Ingeniero de Datos', area:'Tecnología', status:'active', date:'10/04/2022', salary:8800, color:'#22c55e', contractEnd:'2025-10-10', supervisor:4 },
  { id:9, name:'Sofía Herrera', cargo:'Diseñadora UX', area:'Tecnología', status:'inactive', date:'05/01/2024', salary:6500, color:'#e11d48', contractEnd:'2024-05-01', supervisor:8 },
  { id:10, name:'Miguel Flores', cargo:'Jefe de Ventas', area:'Comercial', status:'active', date:'18/08/2020', salary:7800, color:'#3b82f6', contractEnd:'2025-12-15', supervisor:null }
];
...
function saveEmployee(e) {
  ...
  if (idInput) {
    ...
    if (index !== -1) {
      employees[index] = { ...employees[index], name, cargo, area, status, salary, date };
    }
  } else {
    // Create
    const newId = employees.length > 0 ? Math.max(...employees.map(emp => emp.id)) + 1 : 1;
    employees.push({ id: newId, name, cargo, area, status, salary, date, color: randomColor });
  }
...
// Refine expiration check to include a real link/action
function checkContractExpirations() {
  const today = new Date();
  const thirtyDaysFromNow = new Date();
  thirtyDaysFromNow.setDate(today.getDate() + 30);
  ...
  employees.forEach(emp => {
    ...
    if (end > today && end <= thirtyDaysFromNow) {
      const alreadyNotified = notifications.some(n => n.msg.includes(emp.name) && n.type === 'danger');
      if (!alreadyNotified) {
        addNotification('danger', `🚨 **Acción Requerida:** El contrato de **${emp.name}** vence pronto (${emp.contractEnd}). <button class="btn btn-sm btn-primary mt-8" onclick="openRenewalModal(${emp.id})">Tramitar Renovación</button>`);
      }
    }
  });
}
